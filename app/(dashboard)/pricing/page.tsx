    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      <div className="text-center mb-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">Invest in yourself with the plan that fits your goals.</p>
        </motion.div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
        {plans.map((plan, index) => (
          <div key={index} className={'glass glass-hover p-8 rounded-2xl text-center relative ' + (plan.popular ? 'border-space-blue/30' : '')}>
            {plan.popular && <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 bg-gradient-to-r from-space-blue to-space-light rounded-full text-xs font-semibold">Most Popular</div>}
            <div className="flex justify-center mb-4">
              {plan.name === 'Pro' ? <Crown className="w-10 h-10 text-yellow-400" /> : <Sparkles className="w-10 h-10 text-space-blue" />}
            </div>
